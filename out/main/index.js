...
const logger$2 = new Logger("autostart");
function setAutoStart(enabled) {
  try {
    ...
    logger$2.info(`Autostart set to ${enabled}`);
  } catch (err) {
    logger$2.error("Failed to set autostart", err);
  }
}
function getAutoStart() {
  try {
    ...
    logger$2.error("Failed to get autostart status", err);
    return false;
  }
}
...
function handleIPC(settingsStore2, cycleManager2, appMonitor2) {
  ...
  require$$1.ipcMain.handle("settings:set", (_event, partialSettings) => {
    ...
    logger$1.info("Settings updated via IPC");
    return updated;
  });
  ...
  require$$1.ipcMain.handle("autostart:set", (_event, enabled) => {
    ...
    logger$1.info(`Autostart set to ${enabled} via IPC`);
  });
  ...
  require$$1.ipcMain.handle("window:minimize", (event) => {
    ...
    window2?.minimize();
    logger$1.info("Window minimized via IPC");
  });
  ...
  require$$1.ipcMain.handle("app:openLogsFolder", async () => {
    try {
      ...
      logger$1.info("Opened logs folder");
    } catch (error2) {
      const errorMsg = error2 instanceof Error ? error2.message : "Unknown error";
      logger$1.error("Failed to open logs folder", new Error(errorMsg));
      throw error2;
    }
  });
  require$$1.ipcMain.handle("logo:getAvailable", async () => {
    try {
      ...
      const errorMsg = error2 instanceof Error ? error2.message : "Unknown error";
      logger$1.error("Failed to get available media", new Error(errorMsg));
      return [];
    }
  });
  require$$1.ipcMain.handle("logo:upload", async () => {
    try {
      ...
      logger$1.info(`Logo uploaded: ${fileName}`);
      return { success: true, filename: `./${fileName}` };
    } catch (error2) {
      const errorMsg = error2 instanceof Error ? error2.message : "Unknown error";
      logger$1.error("Failed to upload logo", new Error(errorMsg));
      return { success: false, error: errorMsg };
    }
  });
  require$$1.ipcMain.handle("logo:resolvePath", (_, relativePath) => {
    try {
      logger$1.info(`Resolving logo path: ${relativePath}`);
      ...
      if (relativePath.startsWith("http://") || relativePath.startsWith("https://") || relativePath.startsWith("media://")) {
        logger$1.info(`Already absolute: ${relativePath}`);
        return relativePath;
      }
      const resolved = `media://${cleanPath}`;
      logger$1.info(`Resolved ${relativePath} to ${resolved}`);
      return resolved;
    } catch (error2) {
      const errorMsg = error2 instanceof Error ? error2.message : "Unknown error";
      logger$1.error("Failed to resolve logo path", new Error(errorMsg));
      return relativePath;
    }
  });
