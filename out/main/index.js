...
function getResourcesPath(isDirname) {
  if (is.dev) {
    return path$6.join(isDirname, "../../resources");
  } else {
    return process.resourcesPath;
  }
}
function getResourcePath(isDirname, relativePath) {
  const resourcesPath = getResourcesPath(isDirname);
  return path$6.join(resourcesPath, relativePath);
}
  ...
  try {
    ...
    for (const fileName of defaultMedia) {
      ...
      if (!fs$3.existsSync(destPath)) {
        const sourcePath = getResourcePath(__dirname, path$6.join("media", fileName));
        ...
        } else if (fs$3.existsSync(fallbackPath)) {
          await fs$4.copyFile(fallbackPath, destPath);
  ...
  require$$1.protocol.handle("media", async (request) => {
    try {
      ...
      if (!fs$3.existsSync(filePath)) {
        filePath = getResourcePath(__dirname, path$6.join("media", url));
      }
      if (!fs$3.existsSync(filePath)) {
