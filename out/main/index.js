...
class Logger {
  ...
  constructor(context) {
    ...
    this.logDir = require$$0.join(app2.getPath("userData"), "logs");
  ...
  write(level, message) {
    ...
    try {
      ...
      const logFile = require$$0.join(this.logDir, `focuslock-${today}.log`);
  ...
  cleanOldLogs() {
    try {
      ...
      files.forEach((file) => {
        const filePath = require$$0.join(this.logDir, file);
...
class LockWindow {
  ...
  create(lockDurationMs, canSkip) {
    ...
    const iconPath = require$$0.join(__dirname, "../../resources/FocusLock.png");
    displays.forEach((display, index) => {
      ...
      if (is.dev && process.env["ELECTRON_RENDERER_URL"]) {
        ...
        window2.loadURL(`file://${require$$0.join(__dirname, "../renderer/index.html")}?mode=lock&display=${index}`);
      }
